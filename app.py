...
# >>>>> INAYOS ANG ticket_detail FUNCTION <<<<<
...
def ticket_detail(ticket_id):
    ...
    # Ito ang bagong logic: i-populate ang choices bago ang lahat
    department_responses = CANNED_RESPONSES.get(ticket.department, [])
    form.canned_response.choices = [("", "-- Select a response --")] + [(resp, resp) for resp in department_responses]
...
def create_ticket():
    ...
    if form.validate_on_submit():
        ...
        if file_to_save:
            if file_to_save.mimetype != 'application/pdf':
                flash("Invalid file type. Only PDF files are allowed.", "error")
                return redirect(url_for('create_ticket'))

            if file_to_save.content_length and file_to_save.content_length > 25 * 1024 * 1024:
        ...
        new_ticket = Ticket(
            ...
            device_type_other=form.device_type_other.data,
            school_id=form.school_id.data,
            school_name=form.school_name_select.data or form.school_name_text.data,
            personnel_first_name=form.personnel_first_name.data,
            personnel_middle_name=form.personnel_middle_name.data,
            personnel_last_name=form.personnel_last_name.data,
            ...
            existing_email_or_na=form.existing_email_or_na.data,
            remarks=form.remarks.data,
            contact_number=form.contact_number.data,
            document_type_other=form.document_type_other.data,
            position_other=form.position_other.data,
            remarks_other=form.remarks_other.data,
